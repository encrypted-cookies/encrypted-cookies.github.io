...
function nextPage() {
    let totalQuestions = Object.keys(data.exams[randomExam]).length; // get the total number of questions
    if (end + 10 <= totalQuestions) {
        start += 10;
        end += 10;
        document.querySelector('.questionbox').textContent = '';
        displayTen();
    } else {
        // don't do anything if there aren't enough questions left
        return;
    }
}
